...
├── join-member.html           # 正式入會申請書頁面
├── styles.css                 # 樣式表
├── script.js                  # 主要 JavaScript 檔案
├── founder-form.js            # 發起人表單專用 JavaScript 檔案
├── member-form.js             # 會員表單專用 JavaScript 檔案
├── images/                    # 圖片資料夾
│   └── logo.png              # 工會 Logo
└── README.md                  # 說明文件
```
...
### 正式入會申請書 (`join-member.html`)

**步驟流程圖：**
- **Step 1**：線上填表（📝）- 目前步驟
- **Step 2**：加入 LINE（💬）- 傳送身分證
- **Step 3**：審核開通（✅）- 完成入會
...
**差異化設計：**
- **移除發起人元素**：無進度條、無費用減免區塊
- **法律聲明調整**：改為「願申請加入工會」
- **API 設定**：type 改為 'member'（寫入 Members 分頁）
